...
CustomDial::CustomDial(QWidget *_parent) : QWidget (_parent)
{
  ...
  //Задание начальных параметров компонента
...
void CustomDial::setStyle(const CustomDial::TYPE_STYLE &style){
    //Светлый стиль отрисовки
    if(style ==  TYPE_STYLE::SnowWhite)
    {
        m_colors.TextColor                         = Styles::Light::DarkGreen;
        m_colors.BackGroundColor             = Styles::Light::LightGray;
        m_colors.MainCircleColor                = Styles::Light::White;
        m_colors.CentralTextColor              = Styles::Light::Cyan;
        m_colors.CentralCircleColor            = Styles::Light::LightGray;
        m_colors.SwithcerCircleleColorOff  = Styles::Light::Orange;
        m_colors.SwithcerCircleleColorOn   = Styles::Light::Cyan;
        m_colors.BorderColor                       = Styles::Light::Cyan;
    }
    else
    //Тёмный стиль отрисовки
    {
        m_colors.TextColor                         = Styles::Dracula::GrayWhite;
        m_colors.BackGroundColor             = Styles::Dracula::Asphalt;
        m_colors.MainCircleColor                = Styles:: Dracula::DarkGray;
        m_colors.CentralTextColor              = Styles::Dracula::Green;
        m_colors.CentralCircleColor            = Styles::Dracula::Asphalt;
        m_colors.SwithcerCircleleColorOff  = Styles::Dracula::Red;
        m_colors.SwithcerCircleleColorOn   = Styles::Dracula::Green;
        m_colors.BorderColor                      = Styles::Dracula::Green;
    }
...
{
    //Расчёт радиуса индикатора малого
    auto SwitchIndicatorRadius = (getm_sz()/2 + getm_sz()/3) / 2;
    auto BigTickSize = getm_sz()/20;
    //В зависимости от нажатия устанавливаем цвет отрисовки
    if(!getm_pressed())
...
{
  Q_UNUSED(event);
  //Сброс в дефолт значения и расчёт угла отрисовки
...
void CustomDial::paintEvent(QPaintEvent *event)
{
  ...
  for(int i = 0; i < posAngles.size(); i++)
  {
    ...
    //Установка параметров квадрата из выбранного диапазона
  ...
  RectY      = CenterY;
  font.setBold(true);
  //Расчёт ширины и высоты строки для расширения квадратов
